        ...
        <div id="page-wrapper">
            <div class="container-fluid">
                ...
                <div class="row">
					<div class="col-sm-12">
                        <div class="white-box">
                            <h3 class="box-title m-b-0">Transaction Logs Table</h3>
							 <div class="row">
									 ...
									 <div class="login-btn-inner">
                                                                        <div class="row">
                                                                            <div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
                                                                                <button style="float: right" name="search" class="btn btn-sm btn-primary login-submit-cs" type="submit">Search</button>
                                                                            </div>
                                                                        </div>
                                                                    </div>
...
?>ID</th>
                                        <th data-editable="true">Users Name</th>
                                        <th data-editable="true">Farm</th>
                                        <th data-editable="true">Date</th>
                                        <th data-editable="true">Activity</th>
									...
									$select = mysqli_query($con,"select * from transaction_logs where transaction_time BETWEEN '$date_from' AND '$date_to' farm_id='$farm' ");	
									}
							
									else {
										$current_month = date("m");
										$select = mysqli_query($con, "select * from transaction_logs where MONTH(transaction_time)='$current_month' ");
									}
									$sno = 0;
                                    while($results = mysqli_fetch_array($select)) {
										$farms=$results['farm_id'];
										$animal_id=$results['animal_id'];
										$selectfarm=mysqli_query($con,"select* from farms where farmid='$farms' ");
										$selected_farm=mysqli_fetch_array($selectfarm);
                                        $sno++
                                        ?>
                                            <tr>
                                                <input type="hidden" id="id" name="id" value="<?=$results['animal_id'];?>">
                                                <td><?=$sno;?></td>
                                                <td><?=$results['transaction_by'].$today;?></td>
                                                
												<td><?=$selected_farm['farmname'];?></td>
                                                <td><?=$results['transaction_time'];?></td>
                                                <td><?=$results['transaction_action'];?></td>
