            ...
            <div class="row bg-title">
                ...
                <div class="col-lg-9 col-sm-8 col-md-8 col-xs-12">
                    ...
                    <ol class="breadcrumb">
                        ...
                        <li class="active">Expenses Items</li>
                    </ol>
                </div>
                <!-- /.col-lg-12 -->
            </div>
                            ...
                            <table id="example23" class="myTable table table-responsive color-table info-table display nowrap table table-hover table-striped" cellspacing="0" width="100%">
                                ...
                                while($results = mysqli_fetch_array($select)){
                                    ...
                                    <tr><input type="hidden" id="id" name="id" value="<?=$results['id'];?>">
                                        <td><?=$sno;?></td>
                                        <td><?=$results['item'];?></td>
                                        <td><?=$results['cat_id'];?></td>
                                        <td><?=$results['status'];?>
                                            <?php
                                            if($results['status']=="Activated"){
                                                ?>
                                                <a href="settings-process?memberid=<?= $results['id'];?>&&action=<?= "deactivate-item";?>&&subject=<?= $results['item'];?>"  class="btn btn-danger" style="float: right"><i class="fa fa-power-off"></i></a>
                                                <?php
                                                $message = "Success";
                                            }
                                            if($results['status']=="Deactivated"){
                                                ?>
                                                <a href="settings-process?memberid=<?= $results['id'];?>&&action=<?= "activate-item";?>&&subject=<?= $results['item'];?>"  class="btn btn-success" style="float: right"><i class="fa fa-toggle-on"></i></a>
                                                <?php
                                                $message = "Success";
                                            }
                                            ?>
                                        </td>
                                        <td><a  style="color: white" class="btn btn-info"  href="edit-expense-item?id=<?=$results['id'];?>"><i class="fa fa-edit fa-1x"></a></i></td>
                                    </tr>
                                    <?php
                                }
                                ?>
