            ...
            <div class="row bg-title">
                ...
                <div class="col-lg-9 col-sm-8 col-md-8 col-xs-12">
                    ...
                    <ol class="breadcrumb">
                        ...
                        <li class="active">Employees Salary Records</li>
                    </ol>
                </div>
                <!-- /.col-lg-12 -->
            </div>
                    ...
                    <div class="white-box">
                        ...
                        <div class="table-responsive">
                            ...
                            <table id="example23" class="myTable table table-responsive color-table info-table display nowrap table table-hover table-striped" cellspacing="0" width="100%">
                                ...
                                <?php
                                include 'db.php';
                                $select = mysqli_query($con,"select * from s_salary where  farm_id ='$farm'");
                                $sno = 0;
                                while($results = mysqli_fetch_array($select)){
                                    $sno++;
                                    $contact = $results['contact'];
                                    //fetching the total salary

                                    //Selecting the total_salary_paid
                                    $total_salary = mysqli_fetch_array(mysqli_query($con,"select * from employees where tel ='$contact' and farm_id ='$farm'"));
                                    $total_salary = $total_salary['salary'];
                                    //calculating the monthly total payments.
                                    $balance = ($total_salary-$results['amount']);
                                    ?>
                                    <tr><input type="hidden" id="id" name="id" value="<?=$results['animal_id'];?>">
                                        <td><?=$sno;?></td>
                                        <td><?=$results['empname'];?></td>
                                        <td><?=$results['months'];?></td>
                                        <td><?=number_format($total_salary);?></td>
                                        <td><input type="text" id="fname" name="aname" value="<?= number_format($results['amount']);?>"></td>
                                        <td>
                                            <?php
                                            if($balance ==0){
                                                echo "Cleared";
                                            }else{
                                                echo "Shs. ".number_format(($total_salary-$results['amount']));
                                            }
                                            ?>
                                        </td>
